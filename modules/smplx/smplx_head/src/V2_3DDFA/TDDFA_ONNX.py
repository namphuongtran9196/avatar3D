...
class TDDFA_ONNX(object):
    ...
    def __init__(self, **kvs):
        ...
        bfm_onnx_fp = kvs["bfm_fp_onnx"]
        self.bfm_session = onnxruntime.InferenceSession(bfm_onnx_fp, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        ...
        param_mean_std_fp = kvs.get(
            'param_mean_std_fp', make_abs_path(f'weights/param_mean_std_62d_{self.size}x{self.size}.pkl')
        )

        onnx_fp = kvs.get('onnx_fp', kvs.get('checkpoint_fp').replace('.pth', '.onnx'))

        # convert to onnx online if not existed
        self.session = onnxruntime.InferenceSession(onnx_fp, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
