...
def Texture(smplifyx_out_path, front_img_path, back_img_path, model='smplx'):
    ''' data_path: the path to the data, should be like:
            data_path/images/XXX.jpg # image path
            data_path/smpl # output from smplify-x for SMPL model
            data_path/smplx # output from smplify-x for SMPLX model            
    ...
    front_img = os.path.split(os.path.basename(front_img_path))[-1] # remove the path
    back_img = os.path.split(os.path.basename(back_img_path))[-1] # remove the path
    ...
    f_pgn = os.path.join(smplifyx_out_path, 'PGN', '%s_PGN.png'%front_id)
    ...
    b_pgn = os.path.join(smplifyx_out_path, 'PGN', '%s_PGN.png'%back_id)
    ...
    npath = os.path.join(ABS_DIR_PATH, 'outputs', 'texture_%s'%model)
    if f_pgn and b_pgn:
        pgn_path = os.path.join(ABS_DIR_PATH, 'outputs', 'PGN_%s'%model)
    else:
        pgn_path = None
    ...
    shutil.copyfile(f_acc_texture[:-4]+'complete.png',
                    os.path.join(ABS_DIR_PATH, 'outputs', 'texture_%s.png'%model)) 
